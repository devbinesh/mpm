...
/**
 * Repository for the {@link AccountUser} JPA entity. Any custom methods, not already
...
public interface UserRepository extends CrudRepository<AccountUser, Long> {
     ...
     * @return
     */
  //  @Query("select p from AccountUser p where p.username = :username and p.tenant = :tenant")
    AccountUser findByUsernameAndTenant(  String username,String tenant);
}
